...
const navCards = [
  { href: '/programs', label: 'Programs', desc: 'Browse all competition programs', icon: LayoutGrid, color: 'from-emerald-500 to-emerald-700' },
  { href: '/live', label: 'Live', desc: 'Watch the current live program', icon: Radio, color: 'from-rose-500 to-rose-700' },
  { href: '/schedule', label: 'Schedule', desc: 'Full event timetable', icon: Clock, color: 'from-amber-500 to-amber-700' },
  { href: '/participants', label: 'Participants', desc: 'Meet our participants', icon: Users, color: 'from-teal-500 to-teal-700' },
  { href: '/results', label: 'Results', desc: 'Winners and prizes', icon: Trophy, color: 'from-yellow-500 to-amber-600' },
  { href: '/emergency', label: 'Emergency', desc: 'Emergency contacts', icon: Siren, color: 'from-red-500 to-red-700' },
  { href: '/queries', label: 'Queries', desc: 'Get your questions answered', icon: HelpCircle, color: 'from-sky-500 to-sky-700' },
];
...
function useCountdown(target: string | null) {
  ...
  useEffect(() => {
    if (!target) return;
    const targetDate = new Date(target + 'T00:00:00').getTime();
...
export default function HomePage() {
  ...
  const countdown = useCountdown(settings?.event_date ?? null);
  const eventDate = settings?.event_date
    ? new Date(settings.event_date + 'T00:00:00').toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
  ...
  const heroBackground = settings?.banner_url || null;
  const heroLogo = settings?.logo_url || settings?.madrasa_logo || null;
  const headline = settings?.program_name ?? 'Annual Program 2026';
  const subtitle = settings?.program_subtitle ?? 'Annual Program & Competition';
  const venue = settings?.venue ?? 'Madrasa Auditorium';
  const title = settings?.madrasa_name ?? 'Darul Huda Madrasa';
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-islamic-pattern text-white">
        ...
        {heroBackground ? (
          <div className="absolute inset-0">
            <img src={heroBackground} alt="Hero banner" className="h-full w-full object-cover opacity-25" />
          </div>
        ) : null}
          ...
          <div className="mx-auto max-w-3xl text-center">
            ...
            <div className="mb-6 flex justify-center">
              <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl bg-accent/15 text-accent shadow-lg ring-1 ring-accent/20">
                {heroLogo ? (
                  <img src={heroLogo} alt="Madrasa logo" className="h-full w-full object-cover" />
                ) : (
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row sm:gap-8">
              <div className="flex items-center gap-2.5 text-white/90">
                ...
                <span className="text-sm font-medium sm:text-base">{eventDate ?? 'Date to be announced'}</span>
              </div>
              <div className="flex items-center gap-2.5 text-white/90">
                <MapPin className="h-5 w-5 text-accent" />
                <span className="text-sm font-medium sm:text-base">{venue}</span>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
              ...
              {settings?.whatsapp_group_url ? (
                <Button asChild size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white">
                  <a href={settings.whatsapp_group_url} target="_blank" rel="noopener noreferrer">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="mr-2 inline h-4 w-4" fill="currentColor" aria-hidden>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mb-10 text-center">
          <div className="mx-auto mb-4 flex w-fit items-center gap-2 rounded-full bg-primary/5 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-primary">Quick Access</div>
          <h2 className="text-2xl font-bold tracking-tight text-primary sm:text-3xl">Explore the Program</h2>
          <p className="mt-2 text-muted-foreground">Everything you need for the annual program, in one place.</p>
        </div>

        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {navCards.map((card, i) => (
            <Link key={card.href} href={card.href} className="group block animate-fade-up" style={{ animationDelay: `${i * 60}ms` }}>
              <Card className="h-full overflow-hidden border-border/60 bg-white/90 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl">
                <CardContent className="flex h-full flex-col items-start gap-3 p-5">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${card.color} text-white shadow-md transition-transform group-hover:scale-110`}>
