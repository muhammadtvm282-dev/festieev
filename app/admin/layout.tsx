...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const brandName = settings?.madrasa_name || 'Darul Huda Madrasa';

  useEffect(() => {
    if (!loading && !session && pathname !== '/admin') {
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b bg-white px-4 py-3 lg:hidden">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-primary text-primary-foreground">
            {logo ? <img src={logo} alt="Logo" className="h-full w-full object-cover" /> : <Moon className="h-4 w-4" />}
          </div>
          <span className="max-w-[180px] truncate font-bold text-primary">{brandName}</span>
        </div>
        <button onClick={() => setSidebarOpen((v) => !v)} className="rounded-lg p-2 text-primary">
          ...
          <div className="flex h-full flex-col">
            <div className="flex items-center gap-2.5 px-5 py-5">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl bg-accent/20 text-accent">
                {logo ? <img src={logo} alt="Logo" className="h-full w-full object-cover" /> : <Moon className="h-5 w-5" />}
              </div>
              <div className="min-w-0">
                <p className="truncate font-arabic text-base font-bold">{brandName}</p>
